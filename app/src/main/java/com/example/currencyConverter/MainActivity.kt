...
class MainActivity : AppCompatActivity() {
    ...
    private fun calculateResult() {
        if(amountV.text.toString().isEmpty()){
            amountV.setError(getString(R.string.amount_empty_err_msg))
            return
        }
        ...
        try{
            ...
            val amount = amountV.text.toString().toDouble()
            val fromRatio: Double = values[currencyFrom.text.toString()]?.toDouble() ?: 0.0
            val toRatio: Double = values[currencyTo.text.toString()]?.toDouble() ?: 0.0

            val result: Double = amount * toRatio / fromRatio
            val formattedResult = String.format("%.3f", result)

            Log.d(TAG, "VALUES: amount: ${amountV.text}, from: ${currencyFrom.text}, to: ${currencyTo.text}")
            Log.d(TAG, "----")
            Log.d(
                TAG,
    ...
    private fun initViews() {
        currencyFrom = findViewById(R.id.from_autoC)
        currencyTo = findViewById(R.id.to_autoCompleteTextView)
        calcButton = findViewById(R.id.calc_bu)
        amountV = findViewById(R.id.te_amount)
        resultValue = findViewById(R.id.result_autoCompleteTextView2)
    }

    private fun populateDropDownMenu() {
        val countries = listOf(currencyEGP, currencyUSD, currencyEUR, currencyGBP)
        val adapter = ArrayAdapter(this,R.layout.dropdown_text_item, countries)
